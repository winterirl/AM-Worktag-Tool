...
function addDriverPrefix(){
	let prefix = document.getElementById('driverSelect').value;
	document.getElementById('driverInput').value = prefix;
}

function addResourcePrefix(){
	document.getElementById('resourceInput').value = 'RS';
}

function addOptionalPrefix(){
	let prefix = document.getElementById('optionalSelect').value;
	document.getElementById('optionalInput').value = prefix;
}
...
function addCompanyTag(){
	let worktagButtonGroup = document.getElementById('companyButtonGroup');
	let worktagButtons = Array.from(worktagButtonGroup.getElementsByTagName('button'));
	let selectedCompany = document.getElementById('companySelect').value;
...
/**
 * SOM can use 'AC' as a driver tag. Only SOM can do this
 */
...
/**
 * Enables driver input when a driver type is selected. IE, not default. Disables if select changes back to '---'
...
function driverTagSelect(){	
	let currentCompany = document.getElementById('companySelect').value;
...
function driverTagInput(){
	let currentCompany = document.getElementById('companySelect').value;
	let selectedDriver = document.getElementById('driverSelect').value;
	let currentDriver = document.getElementById('driverInput').value;
	let needsResource = requiresResource(currentCompany, selectedDriver);
...
function resourceTagInput(){
	let currentDriver = document.getElementById('driverInput').value;
	let currentResource = document.getElementById('resourceInput').value;
...
function addDriverTag(){
	show('driverRow');
	let worktagButtonGroup = document.getElementById('driverButtonGroup');
	let worktagButtons = Array.from(worktagButtonGroup.getElementsByTagName('button'));
	let selectedDriver = document.getElementById('driverInput').value;
	let driverButton = worktagButtons[1];
	...
	if(!driverButton){ //If there is no driver button create a new element and set properties
		...
		worktagButtonGroup.appendChild(newDriverButton);
	}
...
function addResourceTag(){
	...
	let worktagButtons = Array.from(worktagButtonGroup.getElementsByTagName('button'));
	let selectedDriver = document.getElementById('driverInput').value;
	let selectedResource = document.getElementById('resourceInput').value;
	let driverButton = worktagButtons[1];
	let resourceButton = worktagButtons[2];
	...
	if(!resourceButton){ //If there is no resource button create a new element and set properties
		...
		worktagButtonGroup.appendChild(newResourceButton);
	}
...
//watches optional tag select menu, enables input when selected, adds tag prefix
function optionalTagSelect(){
	let currentOptional = document.getElementById('optionalSelect');
...
//Watches optional tag input and enables button when valid optional tag string exists
function optionalTagInput(){
	let selectedOptional = document.getElementById('optionalInput').value;
...
function noDuplicateTags(){
	...
	//If one of these is a driver we want to disallow the use of them as an optional
	let currentDriver = document.getElementById('driverSelect').value;
	let optionalSelect = document.getElementById('optionalSelect');
...
function addOptionalTag(){
	let addedOptional = document.getElementById('optionalInput').value;
...
function disableAddedTag(tag){
	...
	tagPrefix = tag.substring(0, 2);
	console.log('prefix = ' + tagPrefix);

	for(let j = 0; j < optionalSelect.options.length; j++){
		if(optionalSelect.options[j].value == tagPrefix){
			optionalSelect.options[j].disabled = true;
		}
	}
}
...
function validateTag(tag, tagType){ //Where tag is the actual value of whichever tag, and tagType is the tag's identifier
	...
	if(tagType == 'driver'){
		...
		let optional = document.getElementById('optionalSelect').value;
...
function stringifyWorktag(){
	...
	//let driver = document.getElementById('driverSelect');
	//let needsResource = requiresResource(company, driver);
	let worktagString = '';
	let driverButtonGroup = document.getElementById('driverButtonGroup');
	let driverButtons = Array.from(driverButtonGroup.getElementsByTagName('button'));
	let optionalButtonGroup = document.getElementById('optionalButtonGroup');
...
function submitWorktags(){
	...
	//let submitted = document.getElementById('FieldValue').value;
	//alert("Final Worktag String being submitted is: \n\n" + submitted);
	//document.getElementById('FieldValue').value = submitted;
